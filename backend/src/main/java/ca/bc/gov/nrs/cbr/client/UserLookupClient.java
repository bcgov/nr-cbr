package ca.bc.gov.nrs.cbr.client;
 ...
 * resolves IDIR identities. Replaces the FAM identity-lookup integration CBR
 * used previously — BC Gov SSO (Keycloak) is still where <i>authentication</i> and
 * the CBR roles come from, just not the directory.
 *
 * <p>Base path {@code /api/v1/user-lookup}. Every call carries a Keycloak
 * {@code client_credentials} bearer token minted from CBR's own service
 ...
 *       {@code .client-secret} — CBR's confidential service-account
 *       credentials</li>
