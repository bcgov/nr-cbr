 ...
 * minted for CBR. The {@code azp} (authorized party) check below closes that — a token issued to
...
public class Oauth2SecurityCustomizer implements
   ...
   * {@code FAM:EXPIRES:2026-09-30:CBR_PENG}. Harmless to the exact-match authorisation CBR
   ...
   *
   * <p>Authorities are exposed with no {@code ROLE_} prefix, matching the role codes CBR checks
   * ({@code CBR_ADMIN}, {@code CBR_LEVEL_1}, {@code CBR_PENG}) — see
   * {@link CbrAuthorities}. {@link #FAM_BOOKKEEPING_PREFIX} entries are dropped.
   */
