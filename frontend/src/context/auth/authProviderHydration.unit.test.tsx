...
const Probe = () => {
  ...
  return (
    <div>
      ...
      <span data-testid="privkeys">{Object.keys(user?.privileges ?? {}).join(',') || '-'}</span>
  ...
  it('decodes the DEV token shape into a user', async () => {
    ...
    getUserMock.mockResolvedValue({
      access_token: jwt({
        ...
        azp: 'corporate-bridge-register-0000',
        ...
        client_roles: ['CBR_LEVEL_2', 'CBR_PENG'],
      }),
    });
    ...
    expect(screen.getByTestId('privkeys').textContent).toBe('CBR_LEVEL_2,CBR_PENG');
    expect(screen.getByTestId('roles').textContent).toContain('CBR_PENG');
  });
