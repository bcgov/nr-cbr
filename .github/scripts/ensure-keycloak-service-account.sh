...
# realm for the CBR backend's callouts to the nr-user-lookup-api, with the
...
# Optional:
#   CBR_CLIENT_ID        clientId of the service account to ensure
#                         (default: nr-cbr-backend)
#
...
#
# Keep SCOPES in sync with the endpoints the CBR backend actually calls
# (UserLookupClient: idir-users/search + idir-account-detail). CBR does no
...
CBR_CLIENT_ID="${CBR_CLIENT_ID:-nr-cbr-backend}"
...
echo "Ensuring service-account client: ${CBR_CLIENT_ID}"
...
uuid="$(curl -sS "${auth[@]}" "${clients_url}?clientId=${CBR_CLIENT_ID}" \
  | jq -r '.[0].id // empty')"

if [[ -n "${uuid}" ]]; then
  echo "✓ client exists: ${CBR_CLIENT_ID} (${uuid})"
else
  echo "+ creating client: ${CBR_CLIENT_ID}"
  body="$(jq -n --arg id "${CBR_CLIENT_ID}" '{
    clientId: $id,
    name: "NR CBR backend (user-lookup callouts)",
    description: "Service account for the CBR backend → nr-user-lookup-api. Managed by nr-cbr CI.",
  ...
  if [[ "${code}" != "201" ]]; then
    echo "::error::Failed to create client '${CBR_CLIENT_ID}' (HTTP ${code})."
    exit 1
  fi
  ...
  uuid="$(curl -sS "${auth[@]}" "${clients_url}?clientId=${CBR_CLIENT_ID}" \
    | jq -r '.[0].id // empty')"
  if [[ -z "${uuid}" ]]; then
    echo "::error::Created client '${CBR_CLIENT_ID}' but could not resolve its id."
    exit 1
  fi
  echo "  created (${uuid})"
fi
...
for scope in "${SCOPES[@]}"; do
  ...
  if [[ "${code}" != "204" ]]; then
    echo "::error::Failed to assign scope '${scope}' to '${CBR_CLIENT_ID}' (HTTP ${code})."
    exit 1
  fi
...
secret="$(curl -sS "${auth[@]}" "${clients_url}/${uuid}/client-secret" \
  | jq -r '.value // empty')"
if [[ -z "${secret}" ]]; then
  echo "::error::Could not read the client secret for '${CBR_CLIENT_ID}'."
  exit 1
fi
...
if [[ -n "${GITHUB_OUTPUT:-}" ]]; then
  {
    echo "client_id=${CBR_CLIENT_ID}"
    echo "client_secret=${secret}"
  } >> "${GITHUB_OUTPUT}"
fi

echo "Done. '${CBR_CLIENT_ID}' ready with ${#SCOPES[@]} scope(s); client_id/client_secret exposed as step outputs."
